...
################################################################################
_prompt::init() {
	declare _iris_base_path
	_iris_base_path="$(dirname "$(realpath "${BASH_SOURCE[0]}")")"
...
_iris::help() {
	...
	_iris_version="$(git -C "${_iris_base_path}" describe --tags --abbrev=0)"
	printf -- "iris %s
